...
import { analystQuery } from '../lib/api'
...
export default function AnalystPage() {
  ...
  const runQuery = () => {
    analystApi.call(() => analystQuery(query))
  }
  ...
  return (
    <PageShell
      ...
      subtitle="SQL queries, Pandas analysis, Plotly charts — powered by your database"
    >
      <Tabs
        tabs={[
      ...
      {tab === 'query' && (
        <TwoCol>
          <Card>
            ...
            <Btn onClick={runQuery} loading={analystApi.loading}>
              📊 Run Analysis
            </Btn>
          </Card>
          ...
          <div>
            ...
            <ResultBox
              data={data?.content ? { analysis: data.content, sources: data.sources } : null}
              loading={analystApi.loading}
              error={analystApi.error}
        ...
        <TwoCol>
          <Card>
            ...
            {[
              ...
              { icon: '⚠️', title: 'Anomaly Detection',   desc: 'Flags statistical outliers and trends that deviate from normal patterns' },
            ].map(({ icon, title, desc }) => (
          ...
          <Card>
            ...
            <div style={{ background: '#0f1117', borderRadius: 8, padding: 14 }}>
              {[
                'users', 'sessions', 'messages', 'tool_results',
                'audit_logs', 'hitl_requests', 'billing_subscriptions',
                'ab_experiments', 'scheduler_tasks',
              ].map(t => (
                <div key={t} style={{
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <span style={{ marginLeft: 8, fontSize: 11, color: '#6b7280' }}>
                Agent can only run SELECT queries — no mutations
              </span>
            </div>
          </Card>
        </TwoCol>
